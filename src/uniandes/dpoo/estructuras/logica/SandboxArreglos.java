...
public class SandboxArreglos
{
    ...
    {
    	int tamanoNuevo = arregloEnteros.length +1;
    	int [] Nuevo = new int[tamanoNuevo];
    	
    	for (int i = 0; i < (tamanoNuevo-1);i++){
    		Nuevo[i] = arregloEnteros[i];
    	}
    	Nuevo[tamanoNuevo-1] = entero;
    ...
    {
    	int tamanoNuevo = arregloCadenas.length+1;
    	String[] Nuevo = new String [tamanoNuevo];
    	
    	for (int i = 0; i<(tamanoNuevo-1);i++) {
    		Nuevo[i] = arregloCadenas[i];
    	}
    	
    	Nuevo[tamanoNuevo-1] = cadena;
    ...
    int[] nuevo = new int[tamano-contador];
    int desface=0;
    for (int j = 0; j<tamano;j++) {
    	if (arregloEnteros[j] != valor) {
    		nuevo[j-desface]=arregloEnteros[j];
    		}
    	{desface++;
    ...
    String[] nuevo = new String[tamano-contador];
    int desface=0;
    	    
    for (int j = 0; j<tamano;j++) {
    	
    	if (arregloCadenas[j] != cadena) {
    	nuevo[j-desface]=arregloCadenas[j];
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	for (int i=0;i<tamano;i++) {
    		double dato=Math.floor(valores[i]);
    		nuevo[i]=(int)dato;
    	}
    arregloEnteros=nuevo;
    }
    ...
    public void reiniciarArregloCadenas( Object[] objetos )
    {
    	...
    	for (int i = 0;i<tamano;i++) {
    		String dato = objetos[i].toString();
    		nuevo[i] = dato;
    	}
    arregloCadenas=nuevo;
    ...
    public int contarEnterosRepetidos( )
    {
    	...
    	int[] copia = Arrays.copyOf(arregloEnteros, arregloEnteros.length);
        Arrays.sort(copia);
    	int contador = 0;
        int pos1 = 0;

        while (pos1 < copia.length) {
            int valorAct = copia[pos1];
            int cantidadRepeticiones = 1;
            while (pos1 < copia.length - 1 && copia[pos1 + 1] == valorAct) {
    ...
    public boolean mismosEnteros( int[] otroArreglo )
    {
    	...
    	for (int i = 0; i < copiaOriginal.length; i++) {
             if (copiaOriginal[i] != copiaOtro[i]) {
                 return false;
             }
         }
    	} 
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
    	...
    	for (int i =0; i<cantidad;i++) {
    		...
    		int valorRango=valor*(maximo-minimo+1)+minimo;
    		nuevo[i]=valorRango;
    	}
    	arregloEnteros=nuevo;
    }
